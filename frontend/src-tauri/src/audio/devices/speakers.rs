...
/// Find the built-in speaker/output device (wired, stable, low-latency)
///
...
/// - System audio capture using ScreenCaptureKit (macOS)
/// - Getting audio before Bluetooth encoding (pristine quality)
/// - Fallback when Bluetooth device is default but causes issues
///
/// Note: On macOS, system audio is captured via ScreenCaptureKit regardless
/// of output device, but we use this to determine recording device metadata.
///
        ...
        if let Ok(name) = device.name() {
            ...
            for pattern in &builtin_patterns {
                if name_lower.contains(pattern) {
                    ...
                    // (we want real hardware speakers)
