...
/// Version of the `transcript_sources` eager-backfill migration (D2, change
/// `align-from-immutable-source`). When it is observed newly applied, the
/// per-meeting backfill counts are logged once at upgrade time.
const TRANSCRIPT_SOURCES_BACKFILL_VERSION: i64 = 20260909000000;
...
/// Per-meeting backfill report (design D2), logged once when the
/// `transcript_sources` migration is applied. The count of rows with NULL
/// `token_timestamps` is the replaced-row signature of prior diarization
/// runs — a positive count marks a frozen-degraded source (healed by
/// retranscription).
async fn log_backfilled_null_token_rows(pool: &SqlitePool) {
    ...
    {
        Ok(rows) => {
            let meetings = rows.len();
            for (meeting_id, total, null_tokens) in rows {
                log::warn!(
                    "transcript_sources backfill: meeting {} froze {} row(s), {} with NULL \
                     token_timestamps (replaced-row signature — frozen-degraded source; \
                     retranscription heals)",
                    meeting_id,
                    total,
                    null_tokens
                );
            }
            log::info!("transcript_sources backfill complete: {} meeting(s) seeded", meetings);
        }
        Err(e) => log::warn!("transcript_sources backfill report query failed: {}", e),
    }
}
...
impl DatabaseManager {
    pub async fn new(tauri_db_path: &str, backend_db_path: &str) -> Result<Self> {
        ...
        if !applied_before
            .iter()
            .any(|m| m.version == TRANSCRIPT_SOURCES_BACKFILL_VERSION)
            && applied_now
                .iter()
                .any(|m| m.version == TRANSCRIPT_SOURCES_BACKFILL_VERSION)
        {
            log_backfilled_null_token_rows(&pool).await;
        }
