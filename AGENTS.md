...
**Smoke-test deliverable for UI-affecting changes.** Any OpenSpec change whose outcome users see or trigger in the app — frontend components and views, AND backend-only changes behind a badge, button, or dialog — SHALL add `frontend/e2e/smoke/<change-name>.spec.ts` as an explicit `tasks.md` deliverable, decided at proposal time: either a task lands the spec, or the tasks record WHY the surface is already pinned (link the covering spec). The spec drives the real user flow (click → command dispatch → refetch → visible outcome) through the fail-closed Tauri mock; when the outcome depends on backend state, the spec mutates the fixture inside the mock's command handler to mirror exactly what the real backend persists, then asserts the post-refetch render — see `e2e/smoke/recognized-name-badge-revert.spec.ts` as the pattern. Run the spec before archiving (`npx playwright test e2e/smoke/<change>.spec.ts` from `frontend/`), in addition to the Rust/vitest suites. The local pre-push hook (`.githooks/pre-push`, self-installed via the `prepare` script on `pnpm install`) is the mechanical gate: on `fix/<change>` / `enhance/<change>` branches it runs Vitest plus the branch-scoped spec (`e2e/smoke/<change>.spec.ts`, falling back to the full suite when the scoped spec is absent); on any other branch (main-direct pushes, where the spec name cannot be derived) it runs Vitest plus the FULL Playwright suite — a UI-facing change without E2E coverage fails at push, not in review. `SKIP_SMOKE=1 git push` bypasses. Backfill of smoke tests for pre-existing capabilities is **opportunistic** — added when a capability is next touched, not big-bang; do not block a change on missing coverage elsewhere.
