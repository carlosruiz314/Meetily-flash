...
5. The best cluster differs from the candidate's borrow winner — a deterministic engine-side geometric rule modeling what the render layer's gap borrow would do: reference point = the midpoint of span ∩ gap, distance = point-to-turn-span (containment = 0, start-inclusive/end-exclusive), integer-millisecond arithmetic, ties broken by the earlier turn (the render layer's stable time-order behavior). A turn farther than the borrow cap (GAP_BORROW_MAX_MS) from the reference point is not a winner; when NEITHER flank is within the cap there is no winner and the candidate SHALL splice when gates 1–4 hold AND a confident voiced onset is detected — decided voice evidence replaces geometry that would leave the row unattributed. (The modeled winner is an approximation used solely for contradiction-gating: the render borrow itself fires only on Unknown fragments — fine transcript rows that overlap no turn; the coarse mega-rows this meeting's gate source uses never render as Unknown, and there the rescue-created boundary is what places proportionally-allocated text correctly.)

Splice placement SHALL be the detected VOICE ONSET, not the raw row start: the engine SHALL detect the voiced onset inside span ∩ gap by signal energy/voice-activity (relative to a robust baseline of the gap's own non-voiced level) and start the spliced piece there; the piece ends at the gap edge it coalesces into. Transcript rows on legacy meetings carry locally skewed timestamps (ear-attested on cde5c264: the "Oh, man" row starts ≈1.0s before the voice), so a row-start splice would place a boundary at a time the ear disproves and can regress entries that pass today. If no voiced onset is detected with confidence, the candidate SHALL abstain — an untrimmed splice is forbidden. Onset-detector parameters (baseline statistic, energy threshold, minimum voiced run, analysis hop) are calibratable only under the ear-truth fixture gate's governance, like every other engine tunable. The EMBEDDING window remains the raw span ∩ gap under the slice convention (probe-informed calibration MAY switch identity to the voiced sub-window under gate governance; the voiced sub-window alone MUST NOT lower gate 2's floor).

Candidate evaluation SHALL be a snapshot: all candidates are computed once against the PRE-rescue turn set (the resolve_turns output over the un-augmented piece list), then all selected candidates are spliced in one time-ordered pass as synthetic pieces (`cluster = best`, `margin`, `promoted_subfloor = true`, span = [voice onset, gap end]), and resolve_turns runs again to produce the final turn set. Because the synthetic piece carries `promoted_subfloor = true` with a decided cluster — the same representation as a natively promoted piece — it plays the existing resolve_turns rules with no special-casing, even though the spliced span itself may be far below the promotion floor (gate 2 measures the floor on the RAW span ∩ gap, guarding the text evidence; the flag, not duration, drives the promotion path). The rules it plays: a candidate matching the far flank's cluster coalesces into it (that flank's turn extends across the gap — the surviving turn carries the promotion flag when the candidate founds the turn, and keeps its own flags when the candidate coalesces into it), a candidate differing from both flanks stands as its own low-confidence turn, and `continues_previous` derives per "Persisted turns carry an engine-emitted continuation fact" unchanged. Candidate selection SHALL be a pure, deterministic function (time/index-ordered decisions only; embeddings passed as an ordered slice parallel to the ordered candidate list — no hash-map iteration in the decision path). The number of rescue embeddings SHALL be bounded by the count of text-bearing distinct-turn gaps.
...
#### Scenario: Gap speech attributed with voice-onset placement (S2b)

- **GIVEN** the cde5c264 fixture entry S2b (user-ear-recorded 2026-09-07 via clip_E: Cynthia's "Oh, man" starts ≈15.8s; pin change_at 15.8 ±0.75), where pyannote decodes 14.78–16.12s as silence, the engine's existing 16.12s boundary ALREADY passes the entry, and legacy row skew is ear-attested: the production DB row "Oh, man" [14.78, 15.84] starts ≈1.0s before the voice, while the gate source's covering mega-row makes the engine's raw span ∩ gap [14.78, 16.12] — both windows clear the 0.8s floor
- **WHEN** the engine derives turns with the rescue pass
- **THEN** the raw span ∩ gap embeds to a decided Cynthia identity that contradicts the borrow winner, and the voiced onset is detected at ≈15.8s
- **AND** the spliced piece starts at the onset and coalesces into the right (Cynthia) turn, moving the boundary from 16.12 to ≈15.8 — exactly one change inside the span, within the pinned tolerance
- **AND** S2b still passes (it passed before the rescue; the rescue's user-visible win is the "Oh, man" text rendering under Cynthia instead of the borrowed Carlos badge), and no entry that passed before the rescue regresses

#### Scenario: A skewed row start never becomes a boundary

- **GIVEN** a text-bearing gap whose transcript row starts well before the voiced onset (legacy row-timestamp skew, ear-attested ≈1.0s on cde5c264)
- **WHEN** the rescue pass runs
- **THEN** the spliced piece starts at the detected voiced onset, not at the row start
- **AND** if no voiced onset is detected with confidence, the candidate abstains entirely (an untrimmed row-start splice is forbidden — it would manufacture a boundary at a skewed timestamp)
...
- **GIVEN** a text-bearing distinct-turn gap whose span ∩ gap midpoint is farther than GAP_BORROW_MAX_MS from both flanking turns, whose audio embeds to a decided identity, and whose voiced onset is detected
- **WHEN** the rescue pass runs
