...
- [x] 1.8 **v3 GO recorded 2026-09-07** (logs: `gap-rescue-v3-scan-tuned2-20260907.log`; probe 2 = `gap_rescue_v3_probe.rs`): energy-segmented sub-windows with min 0.12s / +10dB / merge 4 frames (calibrated under the fixture-gate rule; S2b not hold-out) yield, on the target gap: Carlos tail sub-window = left-flank continuation (correct no-op), Cynthia sub-window [15.64,15.82] decided sp1 margin 0.074 ≠ borrow winner sp0 → SPLICE; analytic rehearsal moves the S2b boundary 16.12 → 15.64 (|0.16| ≤ 0.75, improved from |0.32|) with ALL other entries PASS and S2 the unchanged known-limitation; 11 whole-meeting candidates, none touching any fixture span or pin window. Supersedes 1.7's NO-GO for the v3 mechanism.
...
- [x] 4.2 Full chain green via `.bat` runners: lib, synthetic 3/3, ear gate zero FAIL (S2 the sole KNOWN-LIMITATION; S2b PASSING before and after) with render gate clean; evidence logged to `openspec/exploration/`; RENDER line diffed against the recorded baseline (`trustzone-gate-20260906-round2-15of15-render.log`: 237 → 531 → 422) with any fragment-count jump beyond the rescued spans' own rows investigated — DONE 2026-09-08: lib 615 passed/0 failed; synthetic 3/3; ear+render via `render-print-20260908-b.log` (15 passed, 1 known-limitation S2, 0 FAILED; render clean; RENDER-TEXT OK). RENDER diff vs superseded sidecar baseline (237→531→422): authoritative DB-row replay 240→284→243; delta = input change (sidecar mega-rows vs 240 DB rows), documented in no-split-sentences design Context. Evidence: `openspec/exploration/gap-speech-4.2-chain-20260908.log`
