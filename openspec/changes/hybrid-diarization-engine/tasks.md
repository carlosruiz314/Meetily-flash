...
- [ ] 1.2 RED: after 1.1 sign-off, create `frontend/src-tauri/tests/fixtures/ear_truth_cde5c264.json` (kinds: `single_voice`, `voice_change_at`, `distinct_speaker`) and `tests/ear_truth_gate.rs` env-gated test that runs the CURRENT production engine and asserts every entry — must FAIL on the current pipeline, proving detection power
...
- [ ] 4.2 Fallback path (corrupt model) untouched: existing grid + AHC + smoothing tests still green; model-absent skip behavior test pinned ("speaker models not found", no labels)
...
- [ ] 5.2 Recorded gate run: full ear-truth fixture PASS on the new engine, or every non-PASS entry is a KNOWN-LIMITATION with explicit user sign-off; output recorded under `gate-runs/`
