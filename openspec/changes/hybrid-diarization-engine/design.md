...
- TitaNet embeddings are reliable at ≥1.5s (clean slices 0.73–0.94 affinity; the 0.9–1.5s band is marginal, so the floor is set at the spec's `MIN_SPEECH_SECS` 1.5s, not a false 1.4s cliff); sub-1s embeddings are noise. Long-run labeling by majority absorbs voice tails (the 02:12–02:50 defect: voice change ≈163s, ≈3s after the 159s pause).
